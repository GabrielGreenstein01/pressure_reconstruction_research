...
X = fsolve(@(X) solveFunct(X,x,N,P), iniGuessX,options);
...
% - - - - - - - - - - - - - - - - - - - 
function [F dF] = solveFunct(X,x,N,P);
Nn = max(N);
etaHat = X(1:end);
g = 1;
h = 0.5;
...
for j=-Nn:Nn
    ...
    sigma(jIndx,:) =(1/pi).*coeff.*exp(1i*j*x).*cosh(j*(eta+h));
end
...
function dF = jacobian(X,x,N, P)
    ...
    for k = -Nn:Nn
        ...
        for j=-Nn:Nn
            ...
            int = @(x)exp(-1i*j*x)*(c-sqrt(c^2-2.*(P-rho*g*h)));
            coeff = integral(int,0,2*pi,'ArrayValued',true);
            sigma(jIndx,:) =(1/pi).*coeff.*exp(1i*j*x).*sinh(j*(eta+h)).*(j*exp(1i*k*x));
        end
